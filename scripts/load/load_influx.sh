...
INFLUX_URL=${INFLUX_URL:-"http://${DATABASE_HOST:-localhost}:${DATABASE_PORT}"}
INFLUX_URL=${INFLUX_URL%/}

EXE_DIR=${EXE_DIR:-$(dirname $0)}
source ${EXE_DIR}/load_common.sh

until curl "${INFLUX_URL}/ping" 2>/dev/null; do
    echo "Waiting for InfluxDB"
    sleep 1
done

# Remove previous database
curl -X POST "${INFLUX_URL}/query?q=drop%20database%20${DATABASE_NAME}"
# Load new data
