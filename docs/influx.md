...
The loader and the convenience query wrapper accept `TSBS_DATA_ROOT` and
`TSBS_QUERY_ROOT` for artifacts stored outside the source tree. Set
`TSBS_RESULT_ROOT` to place query output in a separate results directory, and
set `INFLUX_URL` when the server is not at `http://localhost:8086`. The older
`BULK_DATA_DIR`, `DATA_FILE`, and positional query-file arguments remain
supported.
